...
The remaining standard library — `Math`, `Date`, `JSON`, the
...
| Matching runs on the host's RegExp engine        | `src/runtime/regexp-compat.js` is the sole host boundary: it compiles an already-validated pattern with the host's own `RegExp`, using the sticky flag so a single attempt at exactly one index reproduces ES5 15.10.2.1 `[[Match]](S, index)` rather than the host's habit of scanning forward. This is sound because the ES5.1 grammar this engine accepts is a strict subset of every host's, so no pattern reaches the host with a meaning the host could get wrong, and the host's boolean/index result is used as-is rather than trusted for anything richer. It is still a real host dependency, the one place in this engine where guest semantics lean on the host's implementation instead of an own one, worth stating plainly rather than leaving implicit.                                                                                                                                                                   |
...
| --------------- | ----------- | -------- | --------- | ------ | ------- |
| Files           | 53,575      | 584      | 584       | 584    | 1.09%   |
| (file, variant) | 102,075     | 1,157    | 1,157     | 1,157  | 1.133%  |
