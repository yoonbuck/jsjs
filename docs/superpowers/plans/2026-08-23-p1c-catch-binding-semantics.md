...
- Start from reviewed design HEAD `558d06c33dc28c294df4145491af1b31e43b42fe`. The implementation branch must retain it as an ancestor.
...
- The reviewed source ledger is `.superpowers/sdd/2026-08-23-p1-decomposition/ledgers/P1C-catch-binding-environments.paths.txt`. Task 4 copies those exact bytes to `tools/test262/es2015-p1c-paths.txt`.
...
- The numeric P1C issue is an execution-time identity from the created native child issue. Never guess a future issue number. Store it in `P1C_ISSUE`, require one exact title match, and pass it into evidence/authority builders as a positive integer.
...
- The pending authority uses `state: "pending"`, `reconciliation: null`, `source.entryLedgerSha256: null`, exact source counts/hash, one `selected-passing` destination for `P1C_ISSUE`, six add-exact evidence files, and no `es5-selection.json` protected output.
...
| Exact P1C runner/evidence       | Create `tools/test262/es2015-p1c.js`, `tools/test262/es2015-p1c-paths.txt`, `test/node/es2015-p1c.test.js`; modify `package.json`, `test/run-node.js`                                                   |
...
| Promotion tests/constants       | Modify `test/node/upstream-select.test.js`, `test/node/es2015-taxonomy.test.js`, `test/node/repository-invariants.test.js`; update focused CI and applied M1 totals in Task 7                           |
...
### Task 1: Admit Exact ES2015 Catch Binding Patterns
...
- [ ] **Step 1: Start the fresh Task 1 worker**
...
- [ ] **Step 5: Implement the exact catch pattern context**
  ...
  Keep this branch limited to identifier leaves; `ObjectPattern` and
  `ArrayPattern` remain structural nodes.
...
- [ ] **Step 6: Implement the exact custom child predicate**
  ...
  admits top-level `AssignmentPattern` and `RestElement`.
...
- [ ] **Step 9: Obtain fresh Task 1 quality review**
...
- [ ] **Step 10: Rerun Task 1 checks and commit**
...
### Task 2: Share Catch Parameter Initialization Across Evaluators
...
- Consumes: Task 1 parser admission, `boundNames`, `newDeclarativeEnvironment`, and `initializeBindingPattern`.
...
- [ ] **Step 1: Start the fresh Task 2 worker**
  ...
  Give the worker Task 1 HEAD, the stable helper signature, the synchronous
...
### Task 3: Prove Fresh Environments, Abrupt Completion, and Realm Parity
...
- [ ] **Step 6: Run the characterization command**
  ...
  Expected after Task 2: PASS. If any case fails, preserve the failing output
...
### Task 4: Add the Exact P1C Runner and Scratch Evidence
...
- Consumes: exact P1C ledger, pinned checkout, low-level Test262 runner,
...
- Produces only ignored scratch before Task 6.
...
- [ ] **Step 2: Copy and verify the exact source ledger**
...
- [ ] **Step 12: Resolve and validate the child issue identity**

  Run only after the P1C native child exists:
  ...
  P1C_ISSUE=$(
    gh issue list \
      --repo yoonbuck/jsjs \
      --state open \
      --search 'Implement ES2015 destructuring catch parameters and catch environments in:title' \
      --json number,title \
      --jq '
        map(select(.title == "Implement ES2015 destructuring catch parameters and catch environments"))
        | if length == 1 then .[0].number else error("expected one exact P1C issue") end
      '
  )
  test "$P1C_ISSUE" -gt 0
  printf '%s\n' "$P1C_ISSUE" \
    > .superpowers/sdd/2026-08-23-p1c-catch-binding/issue-number.txt
  ```
...
- [ ] **Step 14: Obtain fresh Task 4 specification review**
...
- [ ] **Step 15: Obtain fresh Task 4 quality review**
...
- [ ] **Step 16: Rerun Task 4 checks and commit**
...
### Task 5: Register Generic P1C Promotion Without Broadening the Runner
...
- Consumes: Task 4 constants and generic named promotion schema v2.
...
- [ ] **Step 5: Run Task 5 RED commands**
...
- [ ] **Step 9: Run Task 5 GREEN commands**
...
- [ ] **Step 11: Obtain fresh Task 5 quality review**
...
- [ ] **Step 12: Rerun Task 5 checks and commit**
...
### Task 6: Prepare the BASE-Owned Pending P1C Authority

**Files:**

- Modify only on the authority branch: `tools/test262/es2015-provenance.json`
- Read: ignored Task 4 authority scratch

**Interfaces:**

- Consumes: reviewed Tasks 1-5 head, exact six-file hashes, exact protected
  projections, current live `origin/main`, and actual `P1C_ISSUE`.
...
- [ ] **Step 1: Start the fresh Task 6 authority worker**

  The authority worker receives no semantic edit permission. Give it the exact
  pending record from scratch, current BASE SHA, actual issue number, and the
  `roadmap-authority-prepare` marker contract.

- [ ] **Step 2: Freeze the reviewed implementation head and refresh scratch**
...
- [ ] **Step 4: Append only the pending authority**
  ...
  AUTHORITY_WORKTREE=/home/jordan/jsjs/.worktrees/issue78-p1c-authority
  P1C_ISSUE=$(cat \
    "$REPOSITORY_ROOT/.superpowers/sdd/2026-08-23-p1c-catch-binding/issue-number.txt")
  cp \
  ...
  P1C_ISSUE="$P1C_ISSUE" node --input-type=module <<'NODE'
  ...
  const issue = Number(process.env.P1C_ISSUE);
  ...
  if (
    ...
    authority.destinations[0].issue !== issue
  ) {
  ...
  `selected-passing`, null blocker, and the actual `P1C_ISSUE`.
...
- [ ] **Step 5: Prove no BASE repair is required**
  ...
  AUTHORITY_WORKTREE=/home/jordan/jsjs/.worktrees/issue78-p1c-authority
  P1C_ISSUE=$(cat \
    /home/jordan/jsjs/.worktrees/issue78-decomposition/.superpowers/sdd/2026-08-23-p1c-catch-binding/issue-number.txt)
  cd "$AUTHORITY_WORKTREE"
  ...
  mkdir -p \
    /home/jordan/jsjs/.worktrees/issue78-decomposition/.superpowers/sdd/2026-08-23-p1c-catch-binding/task-6
  printf '%s\n' "$AUTHORITY_MARKER" \
    > /home/jordan/jsjs/.worktrees/issue78-decomposition/.superpowers/sdd/2026-08-23-p1c-catch-binding/task-6/authority-marker.txt
  ```
...
- [ ] **Step 7: Obtain fresh Task 6 specification review**
  ...
  actual issue identity, and data-only range.

- [ ] **Step 8: Obtain fresh Task 6 quality/security review**
...
- [ ] **Step 9: Commit, open, verify, and merge the authority PR**
  ...
  ```text
  .superpowers/sdd/2026-08-23-p1c-catch-binding/task-6/authority-merge.txt
  ```
...
### Task 7: Consume P1C Authority and Apply Exact Protected Outputs
...
- [ ] **Step 1: Start the fresh Task 7 consumer worker**
...
- [ ] **Step 2: Refresh exact execution and scratch after rebase**
...
- [ ] **Step 5: Assert exact current-base movements**

  If no foreign roadmap merge intervened, assert:

  ```text
  P1 blocker:       483/951 -> 402/790
  selected-passing: 19,768/37,631 -> 19,849/37,792
  core total:       24,250/46,424 unchanged
  whole tree:       53,575/102,912 unchanged
  selected subset:  20,595/39,139 -> 20,676/39,300
  subset groups:    61 -> 62
  generated paths:  14,272 -> 14,353
  exclusions:       byte-identical
  ```

  If main changed, recompute every global count while requiring the P1C delta
  itself to remain exactly `+81/+161 selected` and `-81/-161 P1`.
...
- [ ] **Step 8: Build and validate the consumer marker**
  ...
  mkdir -p \
    .superpowers/sdd/2026-08-23-p1c-catch-binding/task-7
  printf '%s\n' "$CONSUMER_MARKER" \
    > .superpowers/sdd/2026-08-23-p1c-catch-binding/task-7/consumer-marker.txt
  ```
...
- [ ] **Step 10: Obtain fresh Task 7 specification review**
...
- [ ] **Step 11: Obtain fresh Task 7 quality review**
...
- [ ] **Step 12: Rerun Task 7 checks and commit**
...
### Task 8: Final Review, Exact-Head Delivery, and Closure Evidence

**Files:**

- Modify only for confirmed review findings: files already owned by Tasks 1-7
- Write ignored reports under `.superpowers/sdd/2026-08-23-p1c-catch-binding/task-8/`
...
- [ ] **Step 9: Squash-merge and verify exact main**
  ...
  ```text
  .superpowers/sdd/2026-08-23-p1c-catch-binding/task-8/pr.txt
  .superpowers/sdd/2026-08-23-p1c-catch-binding/task-8/merge-sha.txt
  .superpowers/sdd/2026-08-23-p1c-catch-binding/task-8/ci-run.txt
  .superpowers/sdd/2026-08-23-p1c-catch-binding/task-8/codeql-run.txt
  ```
...
| ------------------------------------------ | -------------------------------- |
| Exact P1C 81/161 ledger/hash               | Tasks 4, 6-8                     |
| Catch parser pattern context               | Task 1                           |
| Source/custom/reusable AST parity          | Task 1                           |
| Duplicate/strict/body lexical early errors | Task 1                           |
| Shared catch parameter environment kernel  | Task 2                           |
| All bindings created before initialization | Task 2                           |
| Sync/generator parity                      | Tasks 2-3                        |
| Per-execution fresh environment            | Task 3                           |
| Iterator close and abrupt completion       | Task 3                           |
| Direct eval variable environment           | Task 3                           |
| Same-Agent cross-Realm value behavior      | Task 3                           |
| Evaluating-Realm errors                    | Task 3                           |
| Complete pinned include closure            | Tasks 4, 7                       |
| Zero exclusion inventory                   | Tasks 4-5, 7                     |
| Bounded runner and entry-point containment | Tasks 4-5                        |
| Focused CI constants                       | Task 5                           |
| Report feature/order preservation          | Tasks 5, 7                       |
| Generic named promotion v2                 | Tasks 4-5, 7                     |
| No BASE repair                             | Task 6                           |
| Separate authority preparation             | Task 6                           |
| Pending-to-applied consumer                | Task 7                           |
| Protected output balance                   | Task 7                           |
| No broad/full local commands               | Global constraints and Tasks 1-8 |
| Fresh worker/spec/quality reviews per task | Review protocol and Tasks 1-8    |
| Exact-head CI/CodeQL and issue evidence    | Task 8                           |
