...
- Consumer accounting recomputes taxonomy and generated absolutes from the live authority BASE and asserts exact P1C deltas (`+81/+161` selected, `-81/-161` P1, `+1` group, `+81` generated paths). The reviewed design-base example is 62 groups / 20,676 selected paths / 39,300 selected variants / 14,353 generated non-T0 paths; it is not a stale-base override.
...
- accepts only an identifier leaf in the pattern-context branch of
  `isSupportedExpressionPosition`;
- adds `CatchClause.param` to `isSupportedNonExpressionPosition`'s `pattern()`
  closure for structural `ObjectPattern`/`ArrayPattern` nodes;
- removes the now-dead non-pattern `CatchClause` expression-position case; and
...
- Modify: `src/parser.js:3260-3335, 3748-3790, 4438-4485, 5285-5635, 6748-6780, 7410-7455`
...
- Produces: structural `ObjectPattern`/`ArrayPattern` placement through
  `isSupportedNonExpressionPosition`'s `pattern()` closure, while identifier
  leaves remain owned by `isSupportedExpressionPosition`.
...
- [ ] **Step 4: Run the parser RED command**
  ...
  requires an identifier and the non-expression `pattern()` placement closure
  does not recognize the catch parameter edge.
...
- [ ] **Step 5: Implement the exact catch pattern context**
  ...
  In `isSupportedNonExpressionPosition`, extend only the existing `pattern()`
  closure:
  ...
  The existing node switch keeps this structural edge limited to
  `ObjectPattern` and `ArrayPattern` because it calls `pattern()` only for
  those two node types and requires `patternContext !== undefined`.
...
- [ ] **Step 8: Obtain fresh Task 2 specification review**
  ...
  expression-leaf and non-expression structural placement, source/custom/
  reusable parity, strict bindings, duplicate names, body lexical collisions,
  invalid top-level pattern nodes, and absence of optional catch binding or
  ES2016 rest-pattern widening.

- [ ] **Step 9: Obtain fresh Task 2 quality review**

  Require review of predicate naming, both placement gates, removal of the dead
  old `CatchClause` expression switch case, error phase, hostile AST behavior,
  cycle/descriptor coverage, and no unrelated parser widening.
...
- [ ] **Step 5: Assert live-base taxonomy deltas and exact generated totals**
  ...
  Independently derive generated expectations from the same live authority
  BASE:

  ```text
  HEAD selected paths = BASE selected paths + 81
  HEAD selected variants = BASE selected variants + 161
  HEAD subset groups = BASE subset groups + 1
  HEAD generated non-T0 paths = BASE generated non-T0 paths + 81
  exclusions = byte-identical to live BASE
  ```

  The reviewed design-base example is:

  ```text
  BASE: 61 groups / 20,595 selected paths / 39,139 selected variants /
        14,272 generated non-T0 paths
  HEAD: 62 groups / 20,676 selected paths / 39,300 selected variants /
        14,353 generated non-T0 paths
  ```

  Compute selected paths with `upstreamSubsetPaths`. Compute selected variants
  from the BASE/HEAD coverage inventory. Compute generated non-T0 paths as the
  unique union of every subset group except
  `es2015/audit-passing-promotion`. Reject overlap, missing P1C roots, a second
  P1C group, or any delta other than the exact P1C movement.

  If live authority BASE does not equal the reviewed example, stop before
  copying projections. Rebuild execution, scratch evidence, the pending
  authority, projection hashes, and reviewed literal integration constants
  from that live BASE. Do not force the old 62 / 20,676 / 39,300 / 14,353
  values onto a different authority base.

  Update the applied-M1 integration totals in
  `test/node/es2015-m1.test.js` only after the P1C promotion exists. Derive the
  literals from live BASE plus the exact P1C deltas; at the reviewed base they
  are:
  ...
  In `test/ci/es2015-syntax-test262.test.js`, derive the reviewed literals from
  live BASE plus 81. At the reviewed base:
  ...
  Mirror the H0/M1 integration completely:
  ...
  This exclusion must appear in `issue25ExpansionPaths` and both broad-policy
  violation derivations; constants alone are insufficient.
...
- [ ] **Step 6: Add generated ownership and documentation**
  ...
  named promotion group, live-base generated deltas, the reviewed-base example
  of 20,676 paths / 62 groups, scratch-only evidence before authority, and the
  local broad/full prohibition.
...
- [ ] **Step 7: Add consumer/projection RED-GREEN tests**
  ...
  - live BASE taxonomy and generated totals produce only exact P1C deltas;
  - a live BASE that differs from the authority/scratch base fails before
    projection copy;
  - focused syntax policy excludes P1C paths from issue-#25 attribution and
    both broad-policy violation lists;
...
- [ ] **Step 10: Obtain fresh Task 8 specification review**
  ...
  empty owner files, no exclusion change, full H0/M1-style focused CI
  attribution filtering, report order, protected operations, pending-to-applied
  transition, live-base delta balance, stale-base rebuild behavior, and
  documentation.
...
| Expression/non-expression placement gates  | Task 2                           |
...
| Full P1C focused-CI attribution filtering  | Task 8                           |
