...
- Resolve and discover dependencies serially in source order. Typed same-loader/same-canonical-id reentrant rejection is guaranteed only during the synchronous dynamic extent of `load(identifier)` invocation. Once `load` returns a `PromiseLike`, later same-identifier requests, including legitimate concurrent roots, deduplicate onto the normal in-flight graph Promise and receive identical record/namespace identity. An async hook must not await or reenter the same loader for the identifier whose source it is producing: that creates an undetectable host-Promise dependency cycle and is a documented `ModuleHost` contract violation, not something the engine portably diagnoses. Different-identifier reentrancy remains allowed and deduplicated. Synchronous typed rejection must not corrupt error/retry cleanup; clear failed resolve/load/parse in-flight work so retries work.
...
Track active load-hook identifiers. Typed same-loader/same-canonical-id rejection with `new ModuleLoaderError({ phase: 'load', identifier, cause })` is guaranteed only while `load(identifier)` is synchronously invoking. Once `load` returns a `PromiseLike`, later same-identifier requests, including legitimate concurrent roots, use the normal in-flight graph Promise and receive identical record/namespace identity. An async hook must not await or reenter the same loader for the identifier whose source it is producing: this undetectable host-Promise dependency cycle is a documented `ModuleHost` contract violation, not a condition the engine portably diagnoses. A different-identifier reentry remains allowed and follows the ordinary deduplicated in-flight maps. On resolve/load/parse rejection, delete the matching in-flight/cache state but retain no failed record; synchronous typed rejection must not corrupt this cleanup. Wrap host/parser failures as `ModuleLoaderError` with the original `cause`; leave typed errors intact.

- [ ] **Step 4: Add reentrancy, validation, identity, and retry tests; run GREEN**

Add focused cases for: accessor/unknown/inherited source-record fields; load receiving the canonical string only; failed resolve and load retry; different-loader and different-Realm nonsharing; synchronous same-id rejection, including cleanup after the nested rejection and outer success; async same-id concurrent-root deduplication with record identity; different-id reentry; source order when the first child’s Promise settles later; and `identifier` absent for an initial resolve failure but present after canonicalization. Do not add a hanging async self-await test.
...
- [ ] `createModuleLoader` obeys canonical loader/Realm identity, source-order acquisition, and validation. Its same-id typed rejection is limited to `load(identifier)`'s synchronous dynamic extent; post-`PromiseLike` same-id concurrent roots deduplicate to identical record/namespace identity; async self-await/reentry is a `ModuleHost` contract violation rather than portably diagnosed; different-id reentrancy remains deduplicated; and synchronous typed rejection preserves error/retry cleanup.
