...
- `foundation-maintenance` is valid only when the base contains the initialized
  ...
  profile before it reads mutable head policy. It permits only the canonical
  manifest path set in `tools/test262/es2015-provenance.json` profile
  `foundation-maintenance`: `.github/workflows/ci.yml`,
  `docs/conformance.md`,
  ...
  authorize any additional path. Generated bytes remain limited to
