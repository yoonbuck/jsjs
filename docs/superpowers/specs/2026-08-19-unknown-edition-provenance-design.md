...
The checked-in taxonomy independently reconstructs the same path bytes and
hash. Tooling must reject a pin, count, order, byte, or hash mismatch before it
can render issues or accept decisions.
