...
The manifest contains one immutable `foundation` profile, one immutable
`foundation-maintenance` profile, and one `decision:<CODE>` profile for every
atomic code. CI and local review use the same fail-closed `--check-range` CLI
with explicit full base/head SHAs.
...
- `foundation-maintenance` is valid only when the base contains the initialized
  foundation. Its first reviewed range bootstraps only from U0 squash
  `8d75b48af2ee7ab04e7c5006980417227ec34568` with canonical manifest SHA-256
  `ad3e55a061f1156fc267655ac8cb977f6a54f934cc56a5efa5689c7fc620ae04`; after
  that, the checker loads the trusted base-tree `foundation-maintenance`
  profile before it reads mutable head policy. It permits only the exact
  maintenance allowlist: provenance tooling/schema/tests, the workflow
  generator/contract files, `docs/conformance.md`, `docs/testing.md`, the
  2026-08-19 and 2026-08-20 design/plan docs, the canonical manifest, and exact
  empty decision fragments. Generated bytes remain limited to
  `.github/workflows/ci.yml` and `tools/test262/es2015-provenance.json`. It
  rejects `src/**`, feature/selection data, taxonomy/audit outputs, non-empty
  decisions, renames, copies, deletions, other paths, head-only profile
  broadening, and an empty range.
...
Every U* PR body has exactly one marker. U0 uses:
...
Decision PRs substitute `decision:<CODE>`. Rendered U* issue bodies also carry
the exact sentence `History, age, path/directory, and source/text similarity
may prioritize review but can never decide edition.` The generated workflow
supplies the actual pull-request base/head and complete body; the unchanged CLI
derives and validates the unique profile marker. Unmarked non-provenance PRs
are neutral, while an unmarked provenance-owned range fails.
