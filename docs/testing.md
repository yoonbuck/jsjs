...
| `TZ=UTC npm run test262:es2015:audit`                                          | Inventories the exact pinned checkout and regenerates the deterministic ES2015 taxonomy.                                                                |
| `TZ=UTC npm run test262:es2015:audit:check`                                    | Writes nothing and fails if the pinned ES2015 taxonomy bytes have drifted.                                                                              |

Run `npm run test262:es2015:audit` and
`npm run test262:es2015:audit:check` only with `TZ=UTC`.
