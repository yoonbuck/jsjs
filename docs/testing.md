...
| `npm run profile:analyze`           | Analyze paired schema-2 CPU/allocation sidecars with checksum correlation and equal-observation, interpreter-only hotspot shares over sample-bearing observations; writes only below `.benchmark-results/` |
...
- `test/node/profile-analysis.test.js` — covers deterministic profile-sidecar
  ...
  sample-bearing equal-observation normalization, and paired atomic analysis
  output replacement.
