...
`foundation-maintenance` profile before head policy is read. Its exact allowlist
is limited to provenance tooling/schema/tests, the workflow generator/contract
files, the canonical manifest, the directly related design/plan/testing/
conformance docs, and exact empty decision fragments. `decision:<CODE>`
