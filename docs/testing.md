...
| `TZ=UTC npm run test262:es2015:provenance`                                     | Rebuild the immutable unknown-edition provenance foundation from the exact pinned taxonomy and reset all 13 decision fragments to empty reviewed ledgers                                                                        |
...
`tools/test262/es2015-provenance.json` plus the 13 empty decision fragments in
`tools/test262/es2015-provenance-decisions/`. `test262:es2015:provenance:check`
is metadata/hash-only: it validates the manifest, immutable hashes, exact
decision-directory membership, and reviewed fragment bytes, but cannot call
`runTest262Suite`. `test262:es2015:provenance:ledger` requires a caller-supplied
...
checkout is missing. The provenance/taxonomy metadata commands
(`npm run test262:es2015:provenance`, `npm run test262:es2015:provenance:check`,
`npm run test262:es2015:audit`, `npm run test262:es2015:audit:check`), the
focused release commands, and the upstream commands (`npm run test262:upstream`,
