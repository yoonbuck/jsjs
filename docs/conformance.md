...
| `Date`     | ES5 call/construct overloads; `Date.parse`, `Date.UTC`, `Date.now`; core ES5 local/UTC accessors and mutators; Annex B `getYear`, `setYear`, and `toGMTString`; `toString`, date/time, locale, UTC/GMT, ISO, `valueOf`, and `toJSON`. Clock and timezone access are confined to a single injectable realm host adapter, which an embedder can supply to make a realm fully deterministic; left unset it defaults to the host clock and host zone, the engine’s only machine-dependent results (see [docs/limitations.md](limitations.md#the-clock-and-the-local-time-zone-come-from-the-host)). Date string parsing implements ES5.1 §15.9.1.15 directly and never calls the host’s `Date.parse`. Locale methods deliberately match their non-locale counterparts. |
