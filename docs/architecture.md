...
`ToString`, `ToObject`, `ToInt32`, `ToUint32`, `ToUint16`,
`CheckObjectCoercible`, plus ES2015's `ToPropertyKey` and `ToLength`.
`ToPrimitive` consults `@@toPrimitive` before `OrdinaryToPrimitive`; `ToNumber`
and `ToString` reject symbols. `ToLength` clamps integer lengths to the
inclusive range from zero through `2^53 - 1`.
...
duplicating catch binding semantics.
...
| `reflect.js`            | The approved Reflect surface: `apply`, `construct`, `defineProperty`, `deleteProperty`, `get`, `getOwnPropertyDescriptor`, `getPrototypeOf`, `has`, `isExtensible`, `ownKeys`, `preventExtensions`, `set`, and `setPrototypeOf`, plus `@@toStringTag`                                                                                                                                                                           |
...
| `shared.js`             | `createNativeFunction`, descriptor/capability helpers, and dense `CreateListFromArrayLike` behavior shared across families                                                                                                                                                                                                                                                                                                      |
...
`Function.prototype.apply`, `Reflect.apply`, and `Reflect.construct` share
`createListFromArrayLike`. It reads `length` once with `ToLength`, then performs
ordered dense indexed `Get` operations so coercion and getter effects happen at
the specification boundary rather than in host arrays.
